...
#      qqnorm(x[[y]], main=y)
#      qqline(x[[y]])
 # }) criar funÁ„o que faz sozinho
par(mfrow=c(1,3),mar = c(5,7,5,5))
qqnorm(medidas$peitoral, main = "QQplot da Vari√°vel 'peitoral'");qqline(medidas$peitoral)
qqnorm(medidas$cintura, main = "QQplot da Vari√°vel 'cintura'");qqline(medidas$cintura)
qqnorm(medidas$quadril, main = "QQplot da Vari√°vel 'quadril'");qqline(medidas$quadril)
...
plot(qchisq((1:nrow(x)-1/2)/nrow(x), df=3),sort(d),
     ...
     ylab = "Dist√¢ncia Ordenada",  main = "Gr√°fico Qui Quadrado")
...
summary(result) #vetor de medias pop masculino È diferente do feminino; apenas a variavel cintura difere entre os grupos
...
summary(pot_pca)
pairs(main = "Rela√ß√£o das Componentes Principais", pot_pca$rotation[,1:4],lower.panel = NULL, xlim=c(-0.8, 0.8),ylim=c(-0.8, 1.2),
      panel=function(x,y, ...){
